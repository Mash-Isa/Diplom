...
def perform_cross_val(grid, param, params, X_tr, y_tr, cl_weight):
    train_auc = []
    test_auc = []
    for x in grid:
        params[param] = x
        ...
        for train_index, test_index in strat.split(X_tr, y_tr):
...
def plot_curves(grid, train_auc, test_auc, plot_xlabel):
...
def perform_tuning(params_values_dict: dict(), X_tr, y_tr, cl_weight=None):
    params = {}
    for param, value in params_values_dict.items():
        grid = value
        train_auc, test_auc, grid_best = perform_cross_val(grid, param, params, X_tr, y_tr, cl_weight)
        params[param] = grid_best
        plot_curves(grid, train_auc, test_auc, param)
    return params
