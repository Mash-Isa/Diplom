...
def plot_scatter(df):
    ...
    def plot_features(x=numerical_columns, y=numerical_columns, color=categorical_columns, log_x=True, log_y=True, plot_button=False):
        if plot_button:
            fig = px.scatter(df, x=x, y=y, color=color, log_x=log_x, log_y=log_y, size_max=20)
            fig.show()

    out = widgets.interactive_output(plot_features,
                                     {'x': x, 'y': y, 'color': color, 'log_x': log_x, 'log_y': log_y, 'plot_button': plot_button})
    display(ui, out)


def kdeplot_single(ax, df, is_0, feature, log_x, disease_name):
...
def plot_distribution(df, column, disease_name):
    ...
    def plot_features(x=numerical_columns, log_x=True, plot_button=False):
        if plot_button:
            ...
            kdeplot_single(ax, df_1, is_0=0, feature=x, log_x=log_x, disease_name=disease_name)
            kdeplot_single(ax, df_0, is_0=1, feature=x, log_x=log_x, disease_name=disease_name)
...
def plot_map(df):
    ...
    plot_button = widgets.ToggleButton(description='Plot Map')
    ui = widgets.VBox([widgets.HBox([x, plot_button])])

    def plot_features(x=categorical_columns, plot_button=False):
        if plot_button:
