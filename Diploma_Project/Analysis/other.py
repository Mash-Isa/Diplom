...
def read_data_from_csv(folder, selected_file):
    file = selected_file.outputs[0]['data']['text/plain'][1:-1]
    path = f'{folder}/{file}'
    df = pd.read_csv(path)
    df.sort_index(axis=1, inplace=True)
    return lower_remove_spaces(df)


def read_data_from_csv_for_model(folder, selected_file):
    file = selected_file.outputs[0]['data']['text/plain'][1:-1]
    path = f'{folder}/{file}'
    df = pd.read_csv(path)
    return df
...
def lower_remove_spaces(df):
    categorical_columns = list(df.select_dtypes(['object']).columns)
    df[categorical_columns] = df[categorical_columns].astype(str).applymap(str.capitalize).applymap(str.strip).replace('Nan', np.nan)
    return df


def format_columns(df, features_to_encode, features_to_drop, have_label=False, label=None, label_name=None):
...
def show_df_info(df):
    ...
    ui = widgets.HBox([show_df_button, show_nrows_ncols_button, show_cols_button, show_info_button, show_cols_stats])

    def buttons_actions(show_df_button=False, show_nrows_ncols_button=False, show_cols_button=False, show_info_button=False, show_cols_stats=False):
        if show_df_button:
            display(df.head(10))
...
def get_model_prediction(model_path, X, thr, model_columns_path):
...
def get_model_decision(model_path, model_columns_path, X, thr):
    proba, _ = get_model_prediction(model_path=model_path, model_columns_path=model_columns_path, X=X, thr=thr)
...
def save_predictions_to_file(model_path, X, thr, model_columns_path, raw_df, new_file_name):
    proba, pred = get_model_prediction(model_path=model_path, model_columns_path=model_columns_path, X=X, thr=thr)
